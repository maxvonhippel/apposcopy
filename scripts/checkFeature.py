...
def checkFeature(dbDir, sqlDir, appDir):
    ...
    myTable={
        'DroidDream':[True]*10,
        'DroidDreamLight':[True]*10,
        'GoldDream':[True]*10,
        'Geinimi':[True]*10,
        'Pjapps':[True]*10,
        'DroidKungFu1':[True]*10,
        'DroidKungFu2':[True]*10,
        'DroidKungFu3':[True]*10,
        'DroidKungFu4':[True]*10,
        'BaseBridge':[True]*10,
        'ADRD':[True]*10,
        'BeanBot':[True]*10,
        'Bgserv':[True]*10,
    ...
    try:
        ...
        for row in rows:
            ...
            if not myTable.has_key(familyName):
                continue
            for i in range(10):
                myTable[familyName][i]=(myTable[familyName][i]\
                 and (features[i]>0))
            print('\n***********************')
            print(output,": ",str(features[0]>0),' ',str(features[1]>0),' ',\
                str(features[2]>0),' ',str(features[3]>0),' ',\
                str(features[4]>0),' ',str(features[5]>0),' ' ,\
                str(features[6]>0),' ',str(features[7]>0),' ',\
                str(features[8]>0),' ',str(features[9]>0),' ',\
                str(features[10]),' ',str(features[11]))
            if nodeId != 'None':
