...
def parseargs():
    parser=argparse.ArgumentParser(\
        description='Post-processing for classification, report generation, \
        and application mapping')
    parser.add_argument('-c', type=str, metavar="<SrcClassFile>", \
        help="Path to src classification file")
    parser.add_argument('-k', type=str, metavar="<SinkClassFile>", \
        help="Path to sink classification file")
    parser.add_argument('-a', type=str, metavar="<APK Dir>", \
        help="Directory to analyze apks")
    parser.add_argument('-m', type=str, metavar="<Results>", \
        help="Dir of flow results for mapping")
    parser.add_argument('-r', type=str, metavar="<Results>", \
        help="Dir of flow results for report")
    parser.add_argument('-o', type=str, metavar="<OutputDir>", \
...
def classifyFlows(app, flows, srcClassDict, sinkClassDict):
    ...
    for f in flows:
        ...
        if sinkC == "offdevice":
            ...
            flowC="other"
        flowClass.append((app,src.replace("$","")\
            .title(),srcClasswoPriority,sink\
            .replace("!","")\
            .title(),sinkC,flowC))
    # Set modifiers
    flowClassModifiers=[]
...
if __name__ == "__main__":
    (apk_dir,resultsDir,srcClassFile,\
        sinkClassFile,outputPath,reportAPKs)=parseargs()
    srcDict={}
    sinkDict={}
    # Set output path
